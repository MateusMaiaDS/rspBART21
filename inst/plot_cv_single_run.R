...
for(jj in 1:11){
  ...
  if(jj <= NCOL(x_train)){
    ...
    scatterplot3d::scatterplot3d(x_train[,1], x_train[,2],
                                 ...
                                 xlab = "X.1", ylab = "X.2", zlab = "f(x.1,x.2)", pch =20)
    scatterplot3d::scatterplot3d(x_train[,1], x_train[,2],
...
main_effects_test_list_norm <- rsp_mod$mcmc$main_effects_test
# PLotting for the test
par(mfrow = c(2,floor(NCOL(x_train)/2)))

for(jj in 1:11){
    if(jj <= NCOL(rsp_mod$data$x_test)){
      plot(rsp_mod$data$x_test[,jj],colMeans(main_effects_test_list_norm[[jj]][n_burn_plot:n_mcmc,, drop = FALSE]),main = paste0('X',jj),
           ylab = paste0('G(X',jj,')'),pch=20,ylim = c(-20,20),xlab = paste0('x.',jj), col = alpha("black",1.0))
    }    else if(jj == 11 ) {
      # plot(x_test[,1], colMeans(main_effects_test_list_norm[[jj]][1:i,,drop = FALSE]), ylab = "G(x.1,x2)", xlab = "X.1")
      # plot(x_test[,2], colMeans(main_effects_test_list_norm[[jj]][1:i,,drop = FALSE]), ylab = "G(x.1,x2)", xlab = "X.1")
      # plot(x_test[,1], x_test[,2], cex = colMeans(main_effects_test_list_norm[[jj]][1000:i,,drop = FALSE]) + colMeans(main_effects_test_list_norm[[1]][1000:i,,drop = FALSE]) + colMeans(main_effects_test_list_norm[[2]][1000:i,,drop = FALSE]),
      #      ylab = "X.2", xlab = "X.1")
      scatterplot3d::scatterplot3d(rsp_mod$data$x_test[,1], rsp_mod$data$x_test[,2],
                                   colMeans(main_effects_test_list_norm[[2]][n_burn_plot:n_mcmc,,drop = FALSE])  + colMeans(main_effects_test_list_norm[[1]][n_burn_plot:n_mcmc,,drop = FALSE]) + (colMeans(main_effects_test_list_norm[[jj]][n_burn_plot:n_mcmc,,drop = FALSE])),
                                   xlab = "X.1", ylab = "X.2", zlab = "f(x.1,x.2)", pch =20)
      # scatterplot3d::scatterplot3d(x_test[,1], x_test[,2],
      #                              (10*sin(x_test[,1]*x_test[,2]*pi)),
      #                              xlab = "X.1", ylab = "X.2", zlab = "f(x.1,x.2)", pch =20)


      # plot(x_test[,2], x_test[,1], cex = 10*sin(pi*x_test[,1]*x_test[,2]), col = "blue",  ylab = "X.2", xlab = "X.1")
...
rmse(x = rsp_mod$y_test_hat[3001:n_mcmc,,drop = FALSE] %>% apply(2,mean), y_test)
mae(x = rsp_mod$y_test_hat[2001:n_mcmc,,drop = FALSE] %>% colMeans(), y_test)

# Running the same model for BART and softbart
bart_mod <- dbarts::bart(x.train = rsp_mod$data$x_train,
                         y.train = rsp_mod$data$y_train,x.test = x_test)

softbart_mod <- SoftBart::softbart(X = rsp_mod$data$x_train,
                         Y = rsp_mod$data$y_train,X_test =  x_test)
